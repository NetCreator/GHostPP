...
//					SendChat(player->GetPID(), "No match found" );
			...
			if( ( Command == "addban" || Command == "ban" || Command == "b" ) && !Payload.empty( ) && !m_GHost->m_BNETs.empty( ) )
			{
				...
				// e.g. "Varlock leaver after dying" -> victim: "Gen", reason: "leaver after dying"
...
//				StopPlayers( "was disconnected (admin ended game)" );
			...
			if( (Command == "fakeplayername" || Command == "fpn") && !m_CountDownStarted )
			{
				string sMsg = Payload;
				if (!sMsg.empty())
				{
					m_FakePlayerName = sMsg;
					
					// Update the fakeplayer's name if it's in the lobby
					if ( m_FakePlayerPID != 255 )
					{
						DeleteFakePlayer( );
						CreateFakePlayer( );
