...
void CGHost :: EventBNETGameRefreshFailed( CBNET *bnet )
{
	if( m_CurrentGame )
	{
		...
		if (m_RehostIfNameTaken)
		{
			...
			m_HostCounter++;
			printf("Host counter taken: incrementing.\n");
			SaveHostCounter();
...
void CGHost :: SetConfigs( CConfig *CFG )
{
	...
	{
		m_FakePlayerName = "FakePlayer";
		CONSOLE_Print( "[GHOST] warning - bot_fakeplayername is longer than 15 characters, using default virtual host name" );
	}
