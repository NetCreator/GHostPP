...
*/
	...
	{
		SendAllChat("If no admin in lobby, to start the game all should type " + m_GHost->m_CommandTrigger + "ready , " + mGHost->m_CommandTrigger + "votestart ");
		m_LastInfoShow = GetTime( );
	} */
...
//				m_LastReservedSeen = GetTime( );
	...
	if (!m_GameOverCanceled)
	{
		...
		if (lessthanminplayers && m_GameOverTime == 0)
		{
			...
			SendAllChat("Game over in 15 seconds, "+ UTIL_ToString(m_Players.size())+" remaining ( < "+UTIL_ToString(m_GHost->m_gameoverminplayers)+" ) "+string(1, m_GHost->m_CommandTrigger)+"override to cancel");
...
void CBaseGame :: EventGameStarted( )
{
	...
	if( m_Replay )
	{
		...
		if( m_FakePlayerPID != 255 )
			m_Replay->AddPlayer( m_FakePlayerPID, m_FakePlayerName );
...
void CBaseGame :: StartCountDownAuto( bool requireSpoofChecks )
{
	if( !m_CountDownStarted )
	{
		...
		{
			if(m_GHost->m_PlayerBeforeStartPrintDelay + 2 > m_ActualyPrintPlayerWaitingStartDelay)
			{
...
void CBaseGame :: UpdateFakePlayer( )
{
	if( m_FakePlayerPID = 255 )
		return;
	...
	for( unsigned char i = 0; i < m_Slots.size( ); i++ )
	{
		if( m_Slots[i].GetPID( ) == m_FakePlayerPID )
		{
			...
			IP.push_back( 0 );
			SendAll( m_Protocol->SEND_W3GS_PLAYERINFO( m_FakePlayerPID, m_FakePlayerName, IP, IP ) );
			m_Slots[i] = CGameSlot( m_FakePlayerPID, 100, SLOTSTATUS_OCCUPIED, 0, m_Slots[i].GetTeam( ), m_Slots[i].GetColour( ), m_Slots[i].GetRace( ) );
			SendAllSlotInfo( );
		}
	}
}
